...
def main():
    ...
    datum = pd.read_csv('ghcnm.v4.0.1.20191006/ghcnm.tavg.v4.0.1.20191003.qfe.dat',header=None, delim_whitespace=True,error_bad_lines=False,engine='python')
    # Read the .inv file full of the collection locations into a dataframe
    locations = pd.read_csv('ghcnm.v4.0.1.20191006/ghcnm.tavg.v4.0.1.20191003.qfe.inv', header=None, delim_whitespace=True, error_bad_lines=False)
    ...
    firstlast(datum,months,kills)
    clean(datum,months,kills)
...
def firstlast(datum, months, kills):
    
    # Move through first and last row of the dataset
    for i in 0,len(datum):
