...
def main():
    ...
    datum = pd.read_csv('ghcnm.v4.0.1.20191005/ghcnm.tavg.v4.0.1.20191005.qfe.dat',header=None, delim_whitespace=True,error_bad_lines=False,engine='python')
    ...
    # List abbreviations for Western Hemisphere countries
    westhem = ['US']
    # Create separate column for country in which collection is located
    datum['country'] = datum['id'].astype(str).str[0:2]
    # Only retain temperature data from western hemisphere countries
    datum = datum.loc[datum['country'].isin(westhem)]
...
def cleanliness(datum,kills,killcount,full,approx):
    ...
    badrows = len(kills)
    good = (12*full)-sum(approx)-sum(killcount)
    ...
    print(sum(approx), "datapoints,", (sum(approx)/(12*full))*100, "percent of the dataset, were invalid but able to be approximated.")
    print(good, "datapoints,", (good/(12*full))*100, "percent of the dataset, were valid.")
